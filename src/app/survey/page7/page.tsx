...
export default async function SurveyPage7({ searchParams }: { searchParams: SearchParamsType }) {
  const resolvedSearchParams = await searchParams;
  const quizUserId = resolvedSearchParams.quiz_user_id;
  const question = await questionService.getQuestionByQuestionNo("6");
  ...
  async function handleSubmit(formData: FormData) {
    ...
    redirect(`/survey/page8?quiz_user_id=${quizUserId}`);
  }
  ...
  return (
    <Layout>
      <SurveyLayout
        ...
        previousLink={`/survey/page6?quiz_user_id=${quizUserId}`}
