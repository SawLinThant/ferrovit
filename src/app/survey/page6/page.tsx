...
export default async function SurveyPage6({ searchParams }: { searchParams: SearchParamsType }) {
  const resolvedSearchParams = await searchParams;
  const quizUserId = resolvedSearchParams.quiz_user_id;
  const question = await questionService.getQuestionByQuestionNo("5");
  ...
  async function handleSubmit(formData: FormData) {
    ...
    redirect(`/survey/page7?quiz_user_id=${quizUserId}`);
  }
  ...
  return (
    <Layout>
      <SurveyLayout
        ...
        previousLink={`/survey/page5?quiz_user_id=${quizUserId}`}
