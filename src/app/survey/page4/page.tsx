...
export default async function SurveyPage4({ searchParams }: { searchParams: SearchParamsType }) {
  const resolvedSearchParams = await searchParams;
  const quizUserId = resolvedSearchParams.quiz_user_id;
  const question = await questionService.getQuestionByQuestionNo("3");
  ...
  async function handleSubmit(formData: FormData) {
    ...
    redirect(`/survey/page5?quiz_user_id=${quizUserId}`);
  }
  ...
  return (
    <Layout>
      <SurveyLayout
        ...
        previousLink={`/survey/page3?quiz_user_id=${quizUserId}`}
